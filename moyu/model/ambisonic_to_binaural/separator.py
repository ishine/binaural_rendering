...
class Separator:
    def __init__(
        ...
        r"""A wrapper for models to take a whole audio as input.

        Args:
            model: nn.Module, trained model
            segment_samples: int, length of segments to be input to a model, e.g., 44100*3 (samplerate * duration)
    ...
    def separate(self, input_dict: Dict) -> np.ndarray:
    ...
    def pad_audio(self, audio: np.ndarray) -> np.ndarray:
        ...
        Returns:
            padded_audio: (channels_num, padded_audio_samples)
        """
    ...
    def enframe(self, audio: np.ndarray, segment_samples: int) -> np.ndarray:
    ...
    def deframe(self, segments: np.ndarray) -> np.ndarray:
